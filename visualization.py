...
def show_visualization():
    ...
    defect_layer = config["defect_layer"] - 1  # 0-indexed
    ...
    ax2.annotate("Tool Body", xy=(x, y), xytext=(xt, yt),
                 arrowprops=dict(arrowstyle="->", color='black'),
                 fontsize=8, color='black')

    # Sensor Pad
    r_sensor = r_inner - pad_gap
    angle = -10
    rad = np.deg2rad(angle)
    ...
    xt = (r_inner + total_thickness + 1.0) * np.cos(rad)
    yt = (r_inner + total_thickness + 1.0) * np.sin(rad)
    ax2.annotate("Sensor Pad", xy=(x, y), xytext=(xt, yt),
                 arrowprops=dict(arrowstyle="->", color='red'),
                 fontsize=8, color='red')

    # Fluid Gap
    ax2.annotate("Fluid Gap", xy=(r_inner, 0),
                 xytext=(r_inner + total_thickness + 1.0, 0),
